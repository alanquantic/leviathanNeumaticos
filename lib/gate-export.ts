...
export function exportGateReportToPDF(data: GateAnalysisData) {
  ...
  doc.setFont('helvetica', 'bold');
  if (autoReject) {
    doc.text('⛔ REJECTED BY LEVIATHAN GATE', 14, 34);
  } else {
  ...
  doc.setTextColor(31, 41, 55);
  doc.setFont('helvetica', 'bold');
  doc.text('Desglose de Puntuación', 14, yPos);
  ...
  if (data.scores) {
    const scoresData = [
      ...
      ['Producción', `${data.scores.production}`, '25'],
  ...
  if (data.executiveSummary.topRisks && data.executiveSummary.topRisks.length > 0) {
    ...
    doc.setFont('helvetica', 'bold');
    doc.text('⚠ Top 5 Riesgos Identificados', 14, yPos);
  ...
  if (data.executiveSummary.topCorrections && data.executiveSummary.topCorrections.length > 0) {
    ...
    doc.setFont('helvetica', 'bold');
    doc.text('✓ Top 5 Correcciones Necesarias', 14, yPos);
  ...
  if (data.executiveSummary.tokenizationLevel) {
    ...
    doc.setFont('helvetica', 'bold');
    doc.text('Nivel de Tokenización', 14, yPos);
  ...
  if (data.baseCaseComparison) {
    ...
    if (data.baseCaseComparison.without_tipping.ebitda_margin_pct < 12) {
      ...
      doc.setTextColor(220, 38, 38); // red-600
      doc.setFont('helvetica', 'bold');
      doc.text('⚠️ EBITDA base case inferior al 12% - Proyecto no viable sin tipping fee contratado', 18, yPos + 6);
      yPos += 10;
    } else {
      yPos += 50;
    }
  ...
  if (data.tippingScoring) {
    ...
    if (data.tippingScoring.penalties_applied && data.tippingScoring.penalties_applied.length > 0) {
      ...
      data.tippingScoring.penalties_applied.forEach((penalty) => {
        ...
        const penaltyText = doc.splitTextToSize(`• ${penalty}`, 180);
    ...
    if (data.tippingScoring.auto_reject_triggered) {
      ...
      doc.setFont('helvetica', 'bold');
      doc.text('🚨 AUTO REJECT: Proyecto no viable sin tipping fee contratado y EBITDA base < 12%', 18, yPos + 6);
      yPos += 15;
    }
  }
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.text('Leviathan Gate — Project Viability Report', 14, 285);
